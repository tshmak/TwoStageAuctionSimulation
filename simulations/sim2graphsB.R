...
filename <- Rfilename("sim2graphsB")
...
roisumm2$ROI <- with(roisumm2, pay2/(direct+indirect) - 1)
...
ggplot(roisumm3, aes(x=as.factor(indirect), y=ROI1)) + ggbar() + 
  ...
  labs(x="Indirect cost", y="Proportion ROI > 0") +
  theme_light()
...
ggplot(roisumm3, aes(x=inflation, y=ROI1)) + 
  ...
  labs(colour="Indirect cost", y="Proportion ROI > 0", shape="k") + 
