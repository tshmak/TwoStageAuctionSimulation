...
summary <- subset(results, q==0)[,list(totalpay=sum(pay), totalvwel=sum(vwel), nprop=nprop[1]), 
                                 by=.(k,n,prop,seed)]
...
roisumm <- subset(results, roi)[,list(pay=mean(pay), direct=mean(direct), q=mean(q), nprop=nprop[1]), 
                                by=.(k,n,prop,b)]
...
ggplot(summary, aes(x=totalpay, y=totalvwel)) + geom_point() + 
  ...
  facet_grid(k~nprop,scales = "free") + rotate

# ggplot(subset(summary, seed %% 10 == 0), aes(x=totalpay, y=totalvwel)) + geom_point() + 
#   geom_abline(slope=1,intercept = 0) + 
...
ggplot(roisumm2, aes(x=b, y=smoothROI, color=as.factor(indirect))) +  
